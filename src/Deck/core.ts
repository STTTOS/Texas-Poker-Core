 ...
 * @returns
 */
const isStraight = (values: Rank[]) => {
  const mapToValue = [...values].map(rankMap).sort((a, b) => a - b);

  const isSpecial = equals(["a", "2", "3", "4", "5"], values);
  const result =
    isSpecial ||
...
const compareFnOfSameType = (a: string, b: string) => {
  ...
  if (ranks1[0] === ranks2[0]) {
    return ranks2[1] - ranks1[1];
  }
  return !!ranks2.length ? ranks2[0] - ranks1[0] : -1;
};
 ...
 * @returns
 */
const compareFn = (a: HandPoke, b: HandPoke) => {
  // 'q13' 'w3+r2'
  const [typeA, typeB] = [getHandPresentation(a), getHandPresentation(b)];
  const [flagA, flagB] = [typeA[0], typeB[0][0]];

  // 最高牌型相同
  if (flagA === flagB) {
    return compareFnOfSameType(typeA, typeB);
  }
 ...
 * @returns
 */
export function getHandPresentation(input: HandPoke) {
  // return
...
// export type HandPresentation = `${handPokeType}${number | ""}${'+'|''}${handPokeType | ''}${number | ""}`;
