...
- [ ] **P4 增量（自愿指令 + 读投影）** — `applyVoluntaryTableCommand`（`Fold|Check|Call|Bet|Raise|AllIn` → `dispatchCommand` + `captureHandReduceProjection`）；`applyFoldOrCheckCommand` 为其别名。下注与交权实现仍在 `Player` + `handBettingActions`，尚未迁入纯 `apply(state, cmd)`。
