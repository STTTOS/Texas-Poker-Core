...
- [x] **P4′（声明式归约 API，迁移期）** — `CanonicalTableSession` + `reduceCanonicalTableSession`：输入为只读数据（`create` + `bootstrap` + `commandSteps`），输出事件链与 `cloneTableSnapshot` 快照序列；**不**返回 `Texas`；实现仍委托可变图。`Texas#setPlayerRoles` 支持 `options.buttonUserId` 以确定性定庄（与先 `setButton` 再 `setPlayerRoles()` 的旧写法对齐）。**仍待**：全量可序列化 `State`、零 `Texas` 的纯规则机、与阶段 6 全量 `reduce(apply)` 合一。
