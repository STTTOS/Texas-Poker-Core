...
- [ ] **P4 增量（自愿指令 + 读投影）** — `applyVoluntaryTableCommand`（`Fold|Check|Call|Bet|Raise|AllIn` → `dispatchCommand` + `captureHandReduceProjection`）；单测 `applyVoluntaryTableCommand.test.ts` 覆盖上述自愿命令 + `Call` 非行动方 / 短码须 `AllIn`；`applyFoldOrCheckCommand` 为其别名；`voluntaryActionDisallowError`；`Player#getAllowedActionsContext` + 根导出 `resolveAllowedActions` / `AllowedActionsContext`；`handBettingActions` 自愿路径经 `resolveAllowedActions(getAllowedActionsContext())`（内部 `voluntaryAllowedActions`）；自愿下注 `resolveVoluntaryBetChipOrError`；跟注 `resolveCallChipsOrError`、加注 `resolveRaiseAdditionalOrError`、全下 `resolveAllInChipsOrError`；`getOthersMaxBetAmountAtCurrentStage` 已委托 `getMaxOthersStageBet`。下注与交权实现仍在 `Player` + `handBettingActions`，尚未迁入纯 `apply(state, cmd)`。
