...
- [ ] **阶段 3** — callback → handler：Core 内已无 `#callbackOfAction` 一类钩子；本仓库提供 `createAppendDomainEventsHandler` 等拼件，**尚未**要求所有持久化/推送只经 `interpret`（需接入方迁移）。
...
- [x] **P4（自愿指令门面 + 纯校验）** — `applyVoluntaryTableCommand` / `isVoluntaryTableCommand`（`Fold|Check|Call|Bet|Raise|AllIn` → `dispatchCommand` + `captureHandReduceProjection`）；`applyVoluntaryTableCommand.test.ts`；`applyFoldOrCheckCommand`；`voluntaryActionDisallowError`；`Player#getAllowedActionsContext` + 根 `resolveAllowedActions` / `AllowedActionsContext`；`handBettingActions` 内 `voluntaryAllowedActions`；`resolveVoluntaryBetChipOrError`、`resolveCallChipsOrError`、`resolveRaiseAdditionalOrError`、`resolveAllInChipsOrError`；`getOthersMaxBetAmountAtCurrentStage` → `getMaxOthersStageBet`。
- [x] **P4′（声明式归约 API，迁移期）** — `CanonicalTableSession` + `reduceCanonicalTableSession`；`TableStateV1` + `applyTableCommandWithStateV1`（单步 `apply` 形：入/出不可变快照 + `eventTrace`，仍委托 `Texas`）；`freezeTableStateV1FromLive` / `reduceCanonicalSessionToTableStateV1`；bootstrap 含 `invite_seat_user`、`lock_seats` / `unlock_seats` / `reset_session`、`flush_pending_turn_handoff` 等；`parseCanonicalTableSessionFromJson` + `schemaVersion: 1`；`parseTableCommandFromJson` / `parseTableCommandFromUnknown`；dev CLI `pnpm replay:canonical <session.json> [--compact]`。**仍待**：全量可序列化 `State`、零 `Texas` 的纯规则机、与阶段 6 全量 `reduce(apply)` 合一。
- [x] **P5（pending 读侧 + 纯模拟）** — `pendingFlowOpsAllowVoluntaryDispatch` / `peekPendingFlowOp` / `simulateDequeuePendingHeadIfMatches`（`pendingFlowReadModel.ts`）；`captureHandReduceProjection` 已复用。Controller 内 **可变** FIFO 与阶段机本体未重写为单测内纯归约（刻意保留，避免大范围行为漂移）。
- [ ] **阶段 6** — 事件持久化与重放：**增量** — 磁带 → 只读复合投影 `projectCompositeReadModel`（根已导出）；**NDJSON append-only**（`appendPersistedRowsToNdjsonFileSync` / `createNdjsonFileDomainEventStore`）+ `domainEventsFromPersistedRows` + `projectCompositeReadModelFromNdjsonFileSync`；新增磁带校验 `validatePersistedDomainEventRows` / `assertPersistedDomainEventRows` 与摘要 `summarizePersistedDomainEventRows`，dev CLI `pnpm replay:composite <file.ndjson>`、`pnpm replay:validate <file.ndjson>`。新增命令牌谱对拍 `verifyCanonicalSessionAgainstPersistedRows` 与 CLI `pnpm replay:verify <session.canonical.json> <events.ndjson> [--compact] [--fail-on-tape-issues-only]`（用于 Replay-First 校验「命令重放」与「事件磁带」一致，或仅巡检磁带结构）。`createAppendDomainEventsHandler` 可接 `interpret` 落盘。**仍缺**：DB 真表、全桌不可变 `reduce(apply)` 状态重放；机器人仍可按现有 API 接 Command。
- [x] **P6（读侧切片）** — `captureSeatUserIdsInActionOrder`（`dealerRingReadModel.ts`，与 `captureTableSnapshot` 环序一致；根 `index` 已导出）；中央池贡献仍经 `Pool#getContributionSnapshot` / 快照 `potTotal`。边池结构、`Texas` 门面瘦身仍属后续演进（§10 表）。
