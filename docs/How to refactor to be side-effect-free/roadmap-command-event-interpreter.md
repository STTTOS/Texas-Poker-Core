...
- [x] **P4′（声明式归约 API，迁移期）** — `CanonicalTableSession` + `reduceCanonicalTableSession`；bootstrap 含 `invite_seat_user`、`lock_seats` / `unlock_seats` / `reset_session`、`flush_pending_turn_handoff` 等；`parseCanonicalTableSessionFromJson` + `schemaVersion: 1`；`parseTableCommandFromJson` / `parseTableCommandFromUnknown`（`TableCommand` 判别校验，供牌谱 JSON 与 wish 入口）；dev CLI `pnpm replay:canonical <session.json> [--compact]`。**仍待**：全量可序列化 `State`、零 `Texas` 的纯规则机、与阶段 6 全量 `reduce(apply)` 合一。
