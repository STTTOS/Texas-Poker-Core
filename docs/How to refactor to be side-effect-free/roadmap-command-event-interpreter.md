...
- [ ] **P4 增量（自愿指令 + 读投影）** — `applyVoluntaryTableCommand`（`Fold|Check|Call|Bet|Raise|AllIn` → `dispatchCommand` + `captureHandReduceProjection`）；`applyFoldOrCheckCommand` 为其别名；`voluntaryActionDisallowError`；`Player#getAllowedActionsContext` + 根导出 `resolveAllowedActions` / `AllowedActionsContext`；自愿下注 `resolveVoluntaryBetChipOrError`；跟注 `resolveCallChipsOrError`、加注 `resolveRaiseAdditionalOrError`、全下 `resolveAllInChipsOrError`；`getOthersMaxBetAmountAtCurrentStage` 已委托 `getMaxOthersStageBet`。下注与交权实现仍在 `Player` + `handBettingActions`，尚未迁入纯 `apply(state, cmd)`。
